const Joi = require('joi');

const promotions_rulesSchema = Joi.object({
  id: Joi.number().integer().positive().required().default('autoincrement('),
  promo_id: Joi.number().integer().positive().allow(null),
  rule_type: Joi.string().valid('count', 'combo', 'itemType', 'amount', 'freeShipping').allow(null),
  threshold: Joi.number().positive().precision(2).required(),
  discount_type: Joi.string().valid('percent', 'amount', 'fixed_price').allow(null),
  discount_value: Joi.number().positive().precision(2).required(),
  created_at: Joi.date().iso().required().default('now('),
  updated_at: Joi.date().iso().required().default('now('),
  @@index([promo_id],: Joi.any().required()
}).options({
  stripUnknown: true,
  abortEarly: false
}).messages({
  'string.empty': '{#label}不能為空',
  'string.min': '{#label}至少需要{#limit}個字符',
  'string.max': '{#label}不能超過{#limit}個字符',
  'any.required': '{#label}為必填項目',
  'number.positive': '{#label}必須大於0',
  'string.email': '請輸入有效的電子郵件地址'
});

module.exports = { promotions_rulesSchema };
