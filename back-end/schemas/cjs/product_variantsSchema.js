...
const product_variantsSchema = Joi.object({
  ...
  @@index([color_id],: Joi.any().required(),
  @@index([flavor_id],: Joi.any().required(),
  @@index([product_id],: Joi.any().required(),
  @@index([size_id],: Joi.any().required()
}).options({
  stripUnknown: true,
  abortEarly: false
}).messages({
...
module.exports = { product_variantsSchema };
