...
const place_category_assignmentsSchema = Joi.object({
  place_id: Joi.number().integer().positive().required(),
  category_id: Joi.number().integer().positive().required(),
  @@id([place_id,: Joi.any().required(),
  @@index([category_id],: Joi.any().required()
}).options({
  stripUnknown: true,
  abortEarly: false
}).messages({
...
module.exports = { place_category_assignmentsSchema };
