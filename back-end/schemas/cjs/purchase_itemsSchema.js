...
const purchase_itemsSchema = Joi.object({
  id: Joi.number().integer().positive().required().default('autoincrement('),
  pur_order: Joi.number().integer().positive().allow(null),
  ...
  @@index([pur_order],: Joi.any().required(),
...
module.exports = { purchase_itemsSchema };
