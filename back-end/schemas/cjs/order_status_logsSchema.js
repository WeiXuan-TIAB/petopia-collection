...
const order_status_logsSchema = Joi.object({
  id: Joi.number().integer().positive().required().default('autoincrement('),
  order_id: Joi.number().integer().positive().allow(null),
  order_status: Joi.string().valid('pending', 'stocking', 'shipped', 'arrived', 'canceled', 'returned').allow(null),
...
module.exports = { order_status_logsSchema };
