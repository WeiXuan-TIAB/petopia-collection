...
export const errorResponse = (res, err) => {
  console.error('[API ERROR]', err?.code, err?.message, err?.meta, err?.stack);
  const payload = (process.env.NODE_ENV === 'production')
    ? { status: 'error', message: '資料庫查詢錯誤' }
    : { status: 'error', message: err?.message, code: err?.code, meta: err?.meta };
