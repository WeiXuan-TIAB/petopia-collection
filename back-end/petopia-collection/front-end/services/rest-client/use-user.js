...
// 取得會員登入狀態
export const useAuthGet = (enabled = true) => {
  // console.log('[DEBUG][useAuthGet] hook triggered, enabled:', enabled)

  const fetcher = async (url) => {
    // console.log('[DEBUG][useAuthGet] fetcher start:', url)
    const res = await fetch(url, { credentials: 'include' })
    // console.log('[DEBUG][useAuthGet] fetcher response status:', res.status)

    if (!res.ok) {
      // console.warn('[DEBUG][useAuthGet] fetcher failed, returning null')
      return null // 401 或其他錯誤時返回 null
    }

    const json = await res.json()
    // console.log('[DEBUG][useAuthGet] fetcher success, data:', json)
    return json
  }

  const { data, error, isLoading, mutate } = useSWR(
    enabled ? `${apiURL}/auth/check` : null,
    fetcher,
    {
  ...
  // console.log('[DEBUG][useAuthGet] SWR return:', {
  //   user,
  //   favorites,
  //   isAuth: !!user?.id,
  //   isLoading,
...
export const useUserProfile = (enabled = true) => {
  const fetcher = async (url) => {
    const res = await fetch(url, { credentials: 'include' })
    if (!res.ok) return null // 401 或其他錯誤回傳 null
    return res.json()
  }

  const { data, error, isLoading, mutate } = useSWR(
    enabled ? `${apiURL}/members/me` : null,
    fetcher,
    {
      shouldRetryOnError: false,
      revalidateOnFocus: false,
...
export const useUserUpdateAvatar = () => {
  const url = isDev
    ? `${apiURL}/members/me/avatar`
    : `${apiURL}/members/me/cloud-avatar`
  const { trigger, isMutating, isError } = useMutation(
...
export const useAuthLogin = () => {
  ...
  const login = async ({ email, password }) => {
    // 轉換成 username 以符合後端 login()
    return await trigger({ data: { username: email, password } })
  }
